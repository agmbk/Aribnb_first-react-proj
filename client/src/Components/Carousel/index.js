		...
		return (
			<div className="container">
						...
						<div style={{cursor: 'pointer', pointerEvents: 'none'}}>
							<svg className={'bin'} width="1280.000000pt" height="1280.000000pt"
								...
								<g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)">
									<path onClick={event => this.props.removeElement( this.props.location )}
				...
				<div className="wrapper">
					...
					<p><strong>{this.props.location.title}</  strong></p>
					<p>{this.formatDate().toLowerCase()}</p>
...
function buttonNext(parent) {
	...
	function event() {
		 ...
		 * Slides in images
		 */
		if (parent.state.imageASrc < parent.props.location.contextualPictures.length - 1) {
	...
	return (
		<button
			style={parent.state.imageASrc === parent.props.location.contextualPictures.length - 1 ? {
				opacity: '0',
				cursor: 'inherit',
			} : null}
